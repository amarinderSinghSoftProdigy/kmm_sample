...
internal class DealsEventDelegate(
    ...
    override suspend fun handleEvent(event: Event.Action.Deals) = when (event) {
        is Event.Action.Deals.GetAllDeals -> getAllDeals(event.page, event.search)
     ...
     */
    private suspend fun getAllDeals(page: Int, search: String) {
        navigator.withScope<DealsScope> {
            val result = withProgress {
                dealsRepo.getAllDeals(page, search, "")
            }

            result.onSuccess { body ->
                it.updateDeals(body.results)
