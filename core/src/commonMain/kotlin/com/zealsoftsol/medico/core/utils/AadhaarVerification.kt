...
internal object AadhaarVerification {
    ...
    fun isValid(aadhaarNo: String): Boolean {
        ...
        if (clearedAadhaarNo.length != 12 || clearedAadhaarNo.first().toInt() in 0..1) return false
        // Check using verhoeff algorithm
        var c = 0
        clearedAadhaarNo.map { it.toString().toInt() }.reversed().forEachIndexed { index, value ->
